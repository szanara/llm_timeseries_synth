...
for i in range(10):
    for j in tqdm(range(6)):        
        with open(f'/home/cz/mds3/COHERE/real/MHAD2/cat{j}-fold{i}.txt', 'r') as file:
            data = file.read()
        if len(data)==0:
            print(f'missing data class {j} fold{i}')
            break
        total_current_generated = len(glob.glob(f'/home/cz/mds3/COHERE/generated/mhad2/cat{j}-fold{i}-*.npy'))
        while total_current_generated < 30:
            co = cohere.Client(api_key=key)
            
            response = co.chat(
              model="command-r-plus",
              message=f"Generate ONE sample with shape (50,3) in the same distribution as the data I provided for you.I want the SAMPLE.Here is the data:{data} "
            )
            gen = response.text
            ...
            try:
                ...
                if array is not None:  # Ensure the array was successfully created
                    ...
                    file_path = f'/home/cz/mds3/COHERE/generated/mhad2/cat{j}-fold{i}-{amount}.npy'
